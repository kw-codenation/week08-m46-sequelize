...
const deleteAuthor = async (req, res) =>     
{
    try 
    {
        ...
        const rows = await Author.destroy({where:req.body})

        res.status(203).json({message: 'author with id ' + JSON.stringify(req.body.id) + ' successfully deleted', rows:rows})
    } 
    catch (error) 
    {
...
const updateAuthor = async (req, res) =>     
{
    ...
    {
        console.log(req.body)
        const rows = await Author.update
                            (req.body.criteria,
                            {where:req.body.where}
                            )

        res.status(200).json({message: 'Update of author id (' + req.body.where.id + ') using criteria ' + JSON.stringify(req.body.criteria) + ' successful', rows:rows})
    } 
    catch (error) 
    {
...
// load authors onto the table 'author'
